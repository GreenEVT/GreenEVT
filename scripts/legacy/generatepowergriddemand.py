"""
This script generates csv file used by the power grid simulator (openDSS)
...
for row in c.execute(
    "SELECT * FROM  buses a LEFT JOIN (SELECT bus_id, sum(vehicles) AS tot_veh FROM simple_parcels WHERE bus_id > 0 GROUP BY bus_id) b ON b.bus_id = a.id "
):
    # All the bus data
